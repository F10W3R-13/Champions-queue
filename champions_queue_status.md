...
파이프라인은 원래 Make.com으로 구축됐으나, **2026-06-19 Make.com 폐기·완전 봇 내장화 완료**. 이제 수집은 **봇 자체가 단독 처리**(`cogs/ingest.py`의 `on_message` 핸들러 + `core.run_ocr`). Make.com 시나리오 4개는 비활성화 + 결제 해지(Free 플랜으로 보존). 프롬프트에는 모드/맵 판별 규칙, "이름은 깨져도 보이는 그대로", K/D/A 파싱, 시간(분:초→초) 변환, 등록 로스터 힌트 등 상세 규칙이 박혀 있음. **검증 완료·가동 중.**
...
### ⏳ 보류/선택
- ~~수집(③)을 Python으로 이전할지~~ → **2026-06-19 완료. 봇에 완전 내장화(`cogs/ingest.py` + `core.run_ocr`), Make.com 결제 해지.**
- layer-1 로스터 강제 OCR 프롬프트 (이미 `ocr_prompt.py`에 roster 힌트로 부분 구현됨).
- reconcile 효율화 → **2026-06-19 완료** (TTL 게이트 + fields projection). 운영자 조회, 모드별 조회, 테스트 데이터 정리는 여전히 보류.
