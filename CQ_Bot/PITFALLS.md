...
14. **OCR 모델 교체 — GPT-5.x 파라미터 비호환 (2026-09-15 발견)**: `gpt-5.6-luna` 전환 시 (a) `max_tokens` 미지원 → `max_completion_tokens` 필수 (gpt-4.1도 지원하므로 공통 사용으로 통일), (b) `temperature`/`top_p`는 기본값 1만 허용 → `run_ocr`이 모델명 `gpt-5` 접두사일 때만 샘플링 파라미터를 생략. **모델 교체 절차**: `OCR_MODEL=<id> python _ocr_model_test.py` 프롬브(1x1 data-URI 이미지로 파라미터만 검증 — 외부 이미지 URL은 fetch 실패로 `invalid_image_url` 오탐 가능) → 통과 시 서버 `.env`에 `OCR_MODEL` 추가 + Apollo Restart. 롤백: env 줄 삭제 후 Restart (기본값 gpt-4.1).
